...
Remember, any class that implements the methods can be played by the bots. (I'm planning on adding Nine Men's Morris later on this year.)

Then, take a look at how the `TicTacToe` and `ConnectFour` classes are actually implemented.
This will help you to understand how to actually implement a game.

After that, start looking at the `position_eval` function (in `position_evaluation.py`) or the .
This will help you to understand how to evaluate games

Next, look at different players such as the `BasicMonteCarloPlayer` (in the `Unused` folder) or the `AdvisedMonteCarloPlayer` (in `advised\_monte\_carlo\_player.py`)
...
Feel free to use the default tic-tac-toe one, or initialize your own and see how well they do!
...
And that's Botter Than You! Please, fool around, play some games, and have some fun.
...
* Allows us to draw games onto the screen without being constrained by HTML and CSS
* Allows us to interact directly with the user - we decide what clicks do

HTML5 Canvas does not come without downsides though:
* Very hard to deal with resizing screens
...
* Why our minimax doesn't use alpha/beta pruning
    * Alpha/beta pruning is useful for speed in the one calculation of the best move to do, but that calculation must be done each move
...
* evaluation.py
    ...
    * All evaluators return an Evaluation object (or a subclass of Evaluation)
        * This includes functions like `monte\_carlo\_eval` and `position_eval`
