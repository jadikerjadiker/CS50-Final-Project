...
class AdvisedMonteCarloPlayer(Player):
    ...
    def __init__(self, mc_simulation_amount, mc_depth, pe_depth, mc_rewards=(1, -1, .5), pe_rewards=(2, -2, .9, 0, 0)):
        ...
        self.pe_func = lambda game, player_number: position_eval(game, player_number, self.pe_depth-1, self.pe_rewards)
    ...
    def make_move(self, game):
        ...
        game.make_move(max([(moves[i], (pe_scores[i], mc_scores[i])) for i in range(len(moves))], key=lambda x: x[1])[0])
...
if __name__ == "__main__":
    ...
    test_against((AdvisedMonteCarloPlayer(6, 2, 6), AdvisedMonteCarloPlayer(5, 2, 4)), ConnectFour, comment=6)